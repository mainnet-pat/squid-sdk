...
class ChangeTracker {
    ...
    async writeChangeRows(changes) {
        let height = new Array(changes.length);
        let currentMempoolIndex = 0;
        if (this.blockHeight <= database_1.mempoolHeight) {
            const result = await this.em.query(`SELECT MAX(index) from ${this.statusSchema}.hot_change_log WHERE block_height <= $1`, [database_1.mempoolHeight]);
        ...
        for (let i = 0; i < changes.length; i++) {
            this.index++;
            index[i] = height[i] <= database_1.mempoolHeight ? this.index + currentMempoolIndex : this.index;
