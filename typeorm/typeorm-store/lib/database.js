...
exports.TypeormDatabase = exports.mempoolHeight = void 0;
...
class TypeormDatabase {
    ...
    async initTransaction(em) {
        ...
        let top = await em.query(`SELECT height, hash FROM ${schema}.hot_block where height >= -1 ORDER BY height`);
    ...
    async getState(em) {
        ...
        let top = await em.query(`SELECT hash, height FROM ${schema}.hot_block where height >= -1 ORDER BY height`);
        return assertStateInvariants({ ...status[0], top });
    }
    transact(info, cb) {
    ...
    transactHot2(info, cb) {
        return this.submit(async (em) => {
            ...
            const topHeight = (0, util_internal_1.maybeLast)(chain)?.height ?? -1;
            const newTopHeight = (0, util_internal_1.maybeLast)(info.newBlocks)?.height ?? -1;
            if (newTopHeight !== topHeight) {
                await this.clearMempool();
            }
    ...
    transactMempool(cb) {
        return this.submit(async (em) => {
            await this.insertHotBlock(em, {
                height: exports.mempoolHeight,
                hash: '0x'
            });
            await this.performUpdates(store => cb(store), em, new hot_1.ChangeTracker(em, this.statusSchema, exports.mempoolHeight));
        });
    }
    clearMempool() {
        return this.submit(em => (0, hot_1.rollbackBlock)(this.statusSchema, em, exports.mempoolHeight));
    }
    ...
    insertHotBlock(em, block) {
        return em.query(`INSERT INTO ${this.escapedSchema()}.hot_block (height, hash) VALUES ($1, $2) ON CONFLICT (height) DO NOTHING;`, [block.height, block.hash]);
    }
